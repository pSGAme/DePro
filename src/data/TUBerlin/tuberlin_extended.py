...
def create_trvalte_splits(args):
    ...
    return {'tr_classes': tr_classes, 'va_classes': va_classes, 'te_classes': te_classes, 'semantic_vec': semantic_vec,
            'cid_mask': cid_mask,
            'splits': splits}
...
def trvalte_per_domain(args, datapath):
    ...
    print(len(classes), len(va_classes), len(te_classes))
    tr_classes = np.setdiff1d(classes, np.union1d(te_classes, va_classes))

    print(len(tr_classes))
